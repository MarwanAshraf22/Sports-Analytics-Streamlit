...
def display_team_report(player_name, start_date, end_date, gps_df, wellness_df, roster_df):
    ...
    metric_data = [
        {"label": "Total Distance", "value": f"{total_distance_card:.2f} meters"},
    ...
    # Section 3: Drill Distribution & Player Comparison
    ...
    fig_drill = px.bar(drill_distribution, x='Drill Name', y='Total Distance', color='Drill Name', title="Distance Covered in Each Drill",
                       color_discrete_sequence=px.colors.qualitative.Plotly)
    
    fig_comparison = px.bar(player_data, x='Player Name', y=['Total Distance', 'Maximum Speed'], barmode='group',
                            title="Player Comparison: Total Distance vs Speed")
    ...
    # Section 4: Distance by Position & Drill
    ...
    # Section 5: Game-Level Comparison
    ...
    fig_total_vs_speed = px.scatter(player_data, x='Total Distance', y='Maximum Speed', color='Player Name',
                                    title="Total Distance vs Max Speed")
    ...
    # Section 8: KPI Distribution Boxplot
    st.header("KPI Distribution per Player")
    fig_kpi_distribution = px.box(filtered_gps, x='Player Name', y='Total Distance', title="Distribution of Total Distance per Player")
    st.plotly_chart(fig_kpi_distribution, key="kpi_distribution_chart")

    # Section 9: Session Time Bubble Chart
